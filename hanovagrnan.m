...
% AUTH: HM, 201408, v1a
% AUTH: HM, 201501, v2ab. Add hAnovaGR, G and R
% AUTH: HM, 201703, v3ab. Deal with NaN
% AUTH: HM, 201703, v3ab1. Add P and DF

% Total var: SST = sum((X(:)-mean(X(:))).^2)
% Residual var: SSE = sum((X-mean(X,2)).^2,1)
% Model var: SSB = sum(size(X,2)*(mean(X,2)-mean(X(:))).^2)
